...
function perform_backups()
{
	...
	do
		FULL_BACKUP_CLAUSE="$FULL_BACKUP_CLAUSE or datname ~ '$FULL_BACKUP_DB'"
	done

	FULL_BACKUP_QUERY="select datname from pg_database where false $FULL_BACKUP_CLAUSE order by datname;"

	echo -e "\n\nPerforming full backups"
	echo -e "--------------------------------------------\n"

	FULL_BACKUP_DB_LIST=`psql -h "$HOSTNAME" -p "$PORT" -U "$USERNAME" -At -c "$FULL_BACKUP_QUERY" postgres`

	echo -e "The following databases were matched for full backup:\n${FULL_BACKUP_DB_LIST}\n"
